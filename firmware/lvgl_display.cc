...
bool LvglDisplay::SetTextFont(std::shared_ptr<LvglFont> text_font) {
    // RECHAZADO POR POLÍTICA (2026-08-07, fix v8 del crash de los 5s):
    // El font del asset (font_noto_sans_common_30_4.bin) viene en un formato
    // de binario LVGL INCOMPATIBLE con nuestro LVGL 9.5 → el lv_font_t
    // resultante tiene get_glyph_dsc/get_glyph_bitmap apuntando a basura
    // (a veces 0xfffffffd, a veces literalmente 0x0). Esa basura NO es
    // detectable con un check de nullptr (por eso IsFontUsable no sirve
    // aquí), y al hacer layout de cualquier label que herede del screen
    // salta InstrFetchProhibited en lv_font_get_glyph_width (PC=0x0).
    // El BUILTIN_TEXT_FONT es el mismo Noto Sans — no perdemos nada por
    // quedarnos siempre con él. Devolver false hace que assets.cc loguee
    // "Ignoring invalid text font asset" y mantenga los builtins.
    ESP_LOGW(TAG, "SetTextFont: rechazado font del asset (binario LVGL incompatible). "
                  "Se mantienen los fonts builtin.");
    return false;
}
