...
// WiFi por defecto para DoveBox: tras un flash completo el NVS queda vacío y
// sin estas credenciales el dispositivo entraría en modo configuración. Se
// siembran solo si no hay NINGUNA credencial en NVS. Vacío = comportamiento stock.
#ifndef CONFIG_DOVEBOX_DEFAULT_WIFI_SSID
#define CONFIG_DOVEBOX_DEFAULT_WIFI_SSID "Xiaomi_A111"
#endif
#ifndef CONFIG_DOVEBOX_DEFAULT_WIFI_PASSWORD
#define CONFIG_DOVEBOX_DEFAULT_WIFI_PASSWORD "4c6nshaqkajh"
#endif
...
void WifiBoard::TryWifiConnect() {
    ...
    // WiFi por defecto (DoveBox): tras un flash completo el NVS queda vacío;
    // en lugar de entrar en modo configuración, sembrar la WiFi de casa.
    // Solo se usa si no hay NINGUNA credencial guardada (respetar lo que el
    // usuario configure después por la app o el AP de configuración).
    if (!have_ssid) {
        const char* default_ssid = CONFIG_DOVEBOX_DEFAULT_WIFI_SSID;
        const char* default_password = CONFIG_DOVEBOX_DEFAULT_WIFI_PASSWORD;
        if (default_ssid[0] != '\0' && default_password[0] != '\0') {
            ESP_LOGI(TAG, "No SSID in NVS, seeding default WiFi %s", default_ssid);
