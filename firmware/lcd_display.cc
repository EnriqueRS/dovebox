...
// Un lv_font_t válido SIEMPRE tiene los callbacks de glyph. Los fonts binarios
// del asset (LvglCBinFont) pueden quedar con estos punteros a basura si el
// binario no es compatible → InstrFetchProhibited en lv_font_get_glyph_width
// al hacer layout de cualquier label que herede esa fuente.
static bool IsFontUsable(const lv_font_t* font) {
...
void LcdDisplay::SetupUI() {
    ...
    auto text_font = lvgl_theme->text_font() ? lvgl_theme->text_font()->font() : nullptr;
    auto icon_font = lvgl_theme->icon_font() ? lvgl_theme->icon_font()->font() : nullptr;
    auto large_icon_font = lvgl_theme->large_icon_font() ? lvgl_theme->large_icon_font()->font() : nullptr;
    if (!IsFontUsable(text_font)) text_font = &BUILTIN_TEXT_FONT;
    if (!IsFontUsable(icon_font)) icon_font = &BUILTIN_ICON_FONT;
    if (!IsFontUsable(large_icon_font)) large_icon_font = &font_material_symbols_30_4;
...
void LcdDisplay::SetEmotion(const char* emotion) {
    ...
    if (image == nullptr) {
        ...
        auto emoji_font_sp = lvgl_theme->emoji_font();
        const lv_font_t* emotion_font = (emoji_font_sp && IsFontUsable(emoji_font_sp->font()))
                                            ? emoji_font_sp->font() : nullptr;
        if (utf8 == nullptr) {
            utf8 = material_symbols_get_utf8(emotion);
            auto large_sp = lvgl_theme->large_icon_font();
            emotion_font = (large_sp && IsFontUsable(large_sp->font())) ? large_sp->font() : nullptr;
        }
        // Fallback robusto: nunca dejar un label con fuente inválida
        // (crash InstrFetchProhibited en lv_font_get_glyph_width al layout).
        if (!IsFontUsable(emotion_font)) {
            auto tf = lvgl_theme->text_font();
            const lv_font_t* tf_font = (tf && IsFontUsable(tf->font())) ? tf->font() : nullptr;
            emotion_font = IsFontUsable(tf_font) ? tf_font : LV_FONT_DEFAULT;
        }
...
void LcdDisplay::SetTheme(Theme* theme) {
    ...
    // Set font (con fallback: si el theme aún no tiene fuentes cargadas o el
    // font del asset no es usable, usar builtin/default para que ningún label
    // herede una fuente inválida). Además REEMPLAZAMOS el font dentro del
    // propio theme (set_text_font) para que cualquier lector posterior
    // (GetThemeFont del dashboard, etc.) obtenga la fuente saneada, no la
    // corrupta del asset.
    auto text_font = lvgl_theme->text_font() ? lvgl_theme->text_font()->font() : nullptr;
    auto icon_font = lvgl_theme->icon_font() ? lvgl_theme->icon_font()->font() : nullptr;
    auto large_icon_font = lvgl_theme->large_icon_font() ? lvgl_theme->large_icon_font()->font() : nullptr;
    if (!IsFontUsable(text_font)) {
        ESP_LOGW(TAG, "SetTheme: text font invalid (asset corrupto), replaced with builtin");
        text_font = &BUILTIN_TEXT_FONT;
        lvgl_theme->set_text_font(std::make_shared<LvglBuiltInFont>(&BUILTIN_TEXT_FONT));
    }
    if (!IsFontUsable(icon_font)) {
        ESP_LOGW(TAG, "SetTheme: icon font invalid (asset corrupto), replaced with builtin");
        icon_font = &BUILTIN_ICON_FONT;
        lvgl_theme->set_icon_font(std::make_shared<LvglBuiltInFont>(&BUILTIN_ICON_FONT));
    }
    if (!IsFontUsable(large_icon_font)) {
        ESP_LOGW(TAG, "SetTheme: large icon font invalid (asset corrupto), replaced with builtin");
        large_icon_font = &font_material_symbols_30_4;
        lvgl_theme->set_large_icon_font(std::make_shared<LvglBuiltInFont>(&font_material_symbols_30_4));
    }
